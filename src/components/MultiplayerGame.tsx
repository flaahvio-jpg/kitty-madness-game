...
export const MultiplayerGame = ({ user, roomId, onLeaveRoom }: MultiplayerGameProps) => {
  ...
  const collectFish = useCallback(async (fishIndex: number) => {
    ...
    try {
      await supabase
        .from('room_players')
        .update({
          fish_collected: currentPlayer.fish_collected + 1
        })
        .eq('room_id', roomId)
        .eq('user_id', user.id);
      ...
      await supabase
        ...
        .insert({
          ...
          user_id: user.id,
          player_name: 'Sistema',
          message: `🐟 ${currentPlayer.player_name} coletou um peixinho!`,
          message_type: 'game'
        });
  ...
  }, [currentPlayer, roomId, user.id]);

  const checkWinCondition = useCallback(() => {
    const allPlayersCollected = players.every(player => player.fish_collected >= 1);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-4">
          ...
          <div className="flex justify-center gap-4 text-sm text-muted-foreground">
            ...
            <span>Jogadores: {players.length}/4</span>
            ...
            <Card className="p-4 game-ui-card">
              ...
              {gameStarted && (
                <div className="flex justify-center gap-6 mb-4 text-lg font-semibold">
                  ...
                  <span className="text-primary">🎯 Nível {room.current_level + 1}</span>
                </div>
              )}
              ...
              {gameStarted && (
                <div className="text-center text-sm text-muted-foreground mt-4">
                  <p>🎮 Use WASD ou setas para mover • Todos devem coletar peixes!</p>
                </div>
              )}
            </Card>
